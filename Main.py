...
def DTCNN():
    ...
    acc = accuracy_score(y_test,predict)*100
    accuracy.append(acc)
    text.insert(END,'Decision Prediction Accuracy : '+str(acc)+"\n")
    ...
    bars = ('Decision Tree Inference','CNN Inference')
    y_pos = np.arange(len(bars))
    plt.bar(y_pos, [accuracy[6],accuracy[7]])
    plt.xticks(y_pos, bars)
    plt.show()
    plt.title('DT & CNN Inference Accuracy Performance Graph')
    plt.show()
...
def graph():
    bars = ('Linear Regression', 'KNN','SVM','CNN','SVM Inference','CNN Inference','KNN Inference','CNN Inference','DT Inference','CNN Inference')
    y_pos = np.arange(len(bars))
...
dtButton = Button(main, text="Run DT & CNN Algorithms", command=DTCNN)
dtButton.place(x=20,y=350)
